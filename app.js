...
async function initApp(user) {
  ...
  const d = today();
  // 주문 입력 화면은 +1일(내일 날짜)로 기본 표시 - 보통 다음날 장사 준비를 위해 입력
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  const dNext = tomorrow.toISOString().slice(0,10);

  document.getElementById('order-date').value = dNext;
...
window.clearDay = function() {
  // ★ 반드시 현재 화면에 보이는 날짜만 초기화
  const date = document.getElementById('order-date').value;

  // 1차 확인
  if(!confirm(`📅 ${date} 하루치 데이터만 초기화합니다.\n\n정말 삭제할까요?`)) return;
  // 2차 확인 (대형사고 방지)
  if(!confirm(`⚠ 마지막 확인!\n\n${date} 데이터가 Firebase에서도 삭제됩니다.\n되돌리기가 불가능합니다.\n\n계속하시겠습니까?`)) return;

  // 초기화 전 undoStack에 스냅샷 저장 (화면 되돌리기용)
  saveUndoSnapshot();

  // ★ 현재 날짜(date)의 화면 state만 초기화
  partners.forEach((_,i) => { state[i] = [{itemId:0,customPrice:null,qty:0}]; });
  ...
  // ★ Firebase에서 현재 날짜(date)만 즉시 삭제 (디바운스 없이 바로 실행)
  clearTimeout(debounceTimer); // 진행 중인 저장 취소
  setSyncStatus('ing', '삭제 중...');
  const delPromises = [];
  if(orders[date]) {
    delete orders[date];
    delPromises.push(deleteDoc(doc(db,'orders',date)));
  }
  if(prices[date]) {
    delete prices[date];
    delPromises.push(deleteDoc(doc(db,'prices',date)));
  }
  Promise.all(delPromises).then(() => {
    setSyncStatus('ok', `${date} 초기화 완료 ✓`);
    updateLastSyncTime();
  }).catch(e => {
    setSyncStatus('err', '삭제 실패 — 다시 시도해주세요');
    console.error('clearDay error:', e);
  });
...
// ===== 날짜 이동 기능 =====
window.moveToDate = function() {
  const fromDate = document.getElementById('order-date').value;
  // 현재 입력된 데이터가 있는지 확인
  const hasData = partners.some((_,pi) => state[pi]?.some(s=>s.itemId>0&&s.qty>0));
  if(!hasData && !orders[fromDate]) { alert('이동할 데이터가 없습니다.'); return; }
  ...
  const targetDate = prompt(
    `현재 화면(${fromDate})의 데이터를 다른 날짜로 이동합니다.\n이동할 날짜를 입력하세요:`,
    nextDate
  );
  if(!targetDate) return;
  if(targetDate === fromDate) { alert('같은 날짜로는 이동할 수 없습니다.'); return; }
  if(!/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) { alert('날짜 형식이 올바르지 않습니다.\n예) 2026-06-15'); return; }

  // 이미 해당 날짜에 데이터가 있으면 경고
  if(orders[targetDate]) {
    if(!confirm(`${targetDate}에 이미 데이터가 있습니다.\n덮어쓸까요?`)) return;
  }

  if(!confirm(`${fromDate} → ${targetDate}\n로 데이터를 이동할까요?`)) return;
  ...
  // 대상 날짜에 저장
  orders[targetDate] = rows;
  // 단가도 이동
  if(Object.keys(todayP).length > 0) {
    prices[targetDate] = Object.assign({}, todayP);
  }

  // 원래 날짜 삭제
  delete orders[fromDate];
  if(prices[fromDate]) delete prices[fromDate];

  // Firebase 저장
  scheduleSync(async () => {
    await saveOrders_c(fromDate, []); // 원래 날짜 삭제
    await saveOrders_c(targetDate, rows); // 새 날짜 저장
    if(prices[targetDate]) await savePrices_c(targetDate, prices[targetDate]);
  });

  // 화면을 이동한 날짜로 전환
  document.getElementById('order-date').value = targetDate;
  loadDay(targetDate);
  alert(`✅ ${targetDate} 로 이동 완료!`);
};
...
window.saveOrder = function() {
  ...
  if(!rows.length){
    // 데이터가 없으면 Firebase에서도 삭제 (빈 날짜 처리)
    if(orders[date]) {
      if(!confirm('이 날짜의 저장된 데이터를 모두 삭제할까요?')) return;
      delete orders[date];
      isDirty=false;
      clearTimeout(debounceTimer);
      setSyncStatus('ing','삭제 중...');
      deleteDoc(doc(db,'orders',date)).then(()=>setSyncStatus('ok','삭제 완료 ✓')).catch(()=>setSyncStatus('err','삭제 실패'));
    } else {
      alert('저장할 데이터가 없습니다.');
    }
    return;
...
let taxHiddenPartners = new Set(); // 세금계산서에서 완전히 숨긴 거래처
let taxSelectorOpen = false;
...
window.runTaxInvoice = function() {
  ...
  // ★ 주문 탭의 거래처 순서(partners 배열)대로, 숨긴 거래처는 완전히 제외
  const fullList = partners
    .filter(name => partnerMap[name])
    .map(name => [name, partnerMap[name]]);
  const list = fullList.filter(([name]) => !taxHiddenPartners.has(name));
  ...
  // 거래처 선택(숨김 관리) 패널 - 토글 가능
  const selectorHtml = `
    <div style="padding:10px 14px;background:#fffbeb;border-bottom:1px solid #fcd34d;">
      <button onclick="toggleTaxSelector()" style="width:100%;display:flex;justify-content:space-between;align-items:center;background:none;border:none;cursor:pointer;padding:0;">
        <span style="font-size:12px;font-weight:500;color:#92400e;">⚙ 거래처 선택 (${hiddenCount>0?`${hiddenCount}곳 숨김`:'전체 표시 중'})</span>
        <span style="font-size:11px;color:#92400e;">${taxSelectorOpen?'접기 ▲':'펼치기 ▼'}</span>
      </button>
      ${taxSelectorOpen ? `
        <div style="margin-top:10px;display:grid;grid-template-columns:repeat(auto-fill,minmax(80px,1fr));gap:6px;max-height:160px;overflow-y:auto;">
          ${fullList.map(([name]) => {
            const isHidden = taxHiddenPartners.has(name);
            return `<label style="display:flex;align-items:center;gap:5px;font-size:12px;padding:5px 7px;background:${isHidden?'#fff':'#fff7d6'};border:1px solid ${isHidden?'#e5e7eb':'#fcd34d'};border-radius:6px;cursor:pointer;">
              <input type="checkbox" ${isHidden?'':'checked'} onchange="toggleTaxPartner('${name}')" style="width:14px;height:14px;cursor:pointer;flex-shrink:0;">
              <span style="font-family:monospace;${isHidden?'color:var(--t3);':''}">${name}</span>
            </label>`;
          }).join('')}
        </div>
        <div style="margin-top:8px;display:flex;gap:6px;">
          <button onclick="taxHiddenPartners.clear();runTaxInvoice();" style="font-size:11px;padding:5px 10px;border-radius:6px;border:1px solid #fcd34d;background:#fff;cursor:pointer;color:#92400e;">전체 선택</button>
        </div>
      ` : ''}
    </div>`;

  let html = selectorHtml + `
    <div style="padding:12px 14px;background:#f8fafc;border-bottom:1px solid var(--bd);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px;">
      <span style="font-size:13px;color:var(--t2);">${y}년 ${parseInt(m)}월 · ${list.length}개 거래처${hiddenCount>0?` (전체 ${fullList.length}곳 중)`:''}</span>
...
window.toggleTaxSelector = function() {
  taxSelectorOpen = !taxSelectorOpen;
  runTaxInvoice();
};

window.toggleTaxPartner = function(name) {
  if (taxHiddenPartners.has(name)) taxHiddenPartners.delete(name);
  else taxHiddenPartners.add(name);
  runTaxInvoice();
};
...
window.copyTaxTable = function(ym) {
  ...
  // 주문 탭 거래처 순서대로 + 숨김 거래처 제외
  const list = partners
    .filter(name => partnerMap[name] && !taxHiddenPartners.has(name))
    .map(name => [name, partnerMap[name]]);
  const total = list.reduce((a,[,v]) => a + v.amt, 0);
  const totalQty = list.reduce((a,[,v]) => a + v.qty, 0);
